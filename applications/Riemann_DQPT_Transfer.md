...
It is not $1/(k_B\mkern3mu 305\ {\rm K})$. The 305 K value describes the
...
$$
  ...
  \mkern5mu },\qquad s=\beta_{\mathrm{eff}}+it.
$$
...
For $\mathrm{Re}s>0$, let

$$
  \eta(s)=\sum_{n=1}^{\infty}(-1)^{n-1}n^{-s}.
$$

On the open critical strip $0<\mathrm{Re}s<1$,
...
$$
  |s|\int_N^{\infty}x^{-\beta-1}\mkern3mu dx
  =\frac{|s|}{\beta}N^{-\beta}.
$$
...
Consequently, on a compact set $K\subset\lbrace\mathrm{Re}s>0\rbrace$, if

$$
  \beta_K=\min_{s\in K}\mathrm{Re}s>0,\qquad
  S_K=\max_{s\in K}|s|,
$$
...
$0<\mathrm{Re}s<1$. Assume $\eta$ has no zero on
$\partial\Omega$, and define

$$
\begin{aligned}
  m_\Omega&=\inf_{s\in\partial\Omega}|\eta(s)|>0,\\
  \beta_\Omega&=\min_{s\in\partial\Omega}\mathrm{Re}s>0,\\
...
$U\Subset\lbrace\mathrm{Re}(s)>0\rbrace$: writing the error as $E_N(s)$, there is
a $C_U$ such that

$$
  |E_N(s)|\leq C_U N^{-\mathrm{Re}(s)-1},
  \qquad s\in U.
$$
...
rate is the continuous function $(1-\mathrm{Re}(s))\log 2$. At a zero,
...
$$
  \mathrm{RH}
  \quad\Longleftrightarrow\quad
  \mathrm{Disc}(\mathcal F_1)
  \subseteq\lbrace s:\mathrm{Re}(s)=1/2\rbrace
  \quad\text{within }0<\mathrm{Re}(s)<1.
$$
...
```math
  \mathrm{Disc}(\mathcal F_1\circ\iota_\beta)
  =
  \lbrace t\in\mathbb R:\zeta(\beta+it)=0\rbrace,
```
...
rate via the Borel decision cell $\lbrace\log2\rbrace$. This statement requires the
...
Choose a compact disk $U\Subset\lbrace0<\mathrm{Re}(s)<1\rbrace$ centered at
...
coherence because $Z_N(\mathrm{Re}(s))>0$, although the normalized
...
| BSC-ZDQ-02a | The alternating eta tail obeys the displayed pointwise and compact-uniform bound on $\mathrm{Re}(s)>0$. | true | proved | N/A | unexecuted | present proof | certified |
...
$$
  q:A\to\lbrace0,1\rbrace,\qquad q(a)=\mathbf 1_{\lbrace0\rbrace}(a).
$$

If there is a $z\in A\setminus\lbrace0\rbrace$ such that $O(z)=O(0)$, then no
decision map $d:Y\to\lbrace0,1\rbrace$ can satisfy $q=d\circ O$ on $A$.
