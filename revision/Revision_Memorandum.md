...
This memorandum records every material repair made while constructing *On
Boundaries of Evidence*. It is separate from the manuscript so that the paper
...
Both Volume I and Volume II tuples are reproduced exactly. The Volume II tuple
is declared the current typed core because the authoritative lineage expressly
adopts it. The later Second Catalogue tuple is treated as a proposed
...
in \(E_\partial\). Existence, representative invariance, linearity, and
boundedness are separate assumptions. The relation \(E_\partial\) is never
used as though it were a linear operator.
...
Second Catalogue explicitly repairs this. The manuscript adopts two
noncompensating gates:

\[
G_\partial\ge g_0,\qquad S_{\rm ext}\le s_0.
\]

These are statistical target diagnostics, not inverse-PDE theorems.
...
For lineage, the Volume II record is reproduced exactly as
...
Thus observable transport is contravariant and
...
It is positive and unital but not generally multiplicative.
...
\(\mathsf E_m\). The source experiment is at least as informative in the
Blackwell sense. The symmetric distance, when used, is
...
Every prefix is tested. Contraction after a violation does not erase the
violation.
...
This gives the stated category theorem under inspectable hypotheses. Mixed
modes require a separate comparison functor, and stochastic equality-gluing
is unavailable unless the decoration functor supplies it; no disintegration
is inferred from a pushout.
...
The states \(|+0\rangle\langle+0|\) and
\(|00\rangle\langle00|\) have the same system marginal but give \(I/2\) and
\(|0\rangle\langle0|\) after transform and frame discard. This pair proves
nonfactorization through the reduced quotient on the stated class. A
discard--reset--transform route is also displayed. The example is a
finite-group QRF fixture, not a universal QRF theorem.
...
The first is a topological pairing. The second requires a physically derived
map with units and normalization.
...
- a solenoidal confinement analogy.
...
One confidence score is replaced by five independent axes:

1. mathematical;
2. empirical;
3. computational;
4. source;
5. transfer/deployment.

A dependency edge names the axes it transmits. Demotion is the axiswise meet
over required predecessors. A successful theorem or experiment does not
promote an unrelated node.
...
1. A fully constructed category or double category combining physical
   interfaces with the new observable-complete morphism record.
